...
// Public: client needs this to load PayPal SDK (no secret)
...
router.post('/capture-order', verifyToken, async (req, res) => {
  try {
    ...
    if (order.client_id !== req.user.client_id) return res.status(403).json({ error: 'Not your order' });
    ...
    if (!captureRes.ok) {
      return res.status(400).json({ error: captureData.message || captureData.details?.[0]?.description || 'PayPal capture failed' });
    }
    ...
    await updateClientListingPositions(order.client_id, order.client_role_id).catch((err) => console.error('[capture-order] updateClientListingPositions:', err.message));
    if (planName === 'Diamond') {
      await addClientServicesToAdvertList(order.client_id).catch((err) => console.error('[capture-order] addClientServicesToAdvertList:', err.message));
    }
    res.json({
      success: true,
      plan: planName,
